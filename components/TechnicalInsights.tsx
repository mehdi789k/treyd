...
interface TechnicalInsightsProps {
    ...
    highlightedPattern: AnalysisResult['patterns'][0] | null;
    onPatternSelect: (pattern: AnalysisResult['patterns'][0] | null) => void;
}
            ...
            <ul className="space-y-3 text-gray-400 text-sm">
                {patterns.length > 0 ? patterns.map((p, i) => (
                    <li 
                        ...
                        className={`flex items-start gap-3 p-3 rounded-md bg-gray-700/20 border transition-all duration-200 cursor-pointer 
